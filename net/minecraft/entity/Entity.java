...
import my.distance.module.modules.combat.HitBox;
...
public abstract class Entity implements ICommandSender
{
    ...
    public float getDistanceXZToEntity(Entity entityIn) {
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            if (this == Minecraft.getMinecraft().thePlayer) {
                EventBus.getInstance().register(em);
            }
            if (em.getStepHeight() > 0.0F && flag1 && (d3 != x || d5 != z))
            {
                ...
                if (d11 * d11 + d8 * d8 >= x * x + z * z)
                {
                    ...
                    EventBus.getInstance().register(new EventStepConfirm());
                }
                if (this == Minecraft.getMinecraft().thePlayer)
                    em = new EventStep(false, stepHeight, 1D + y);
                EventBus.getInstance().register(em);
            }
            ...
            if (this.canTriggerWalking() && !(flag && !SafeWalkUtil.flag) && this.ridingEntity == null)
            {
    ...
    {
        if (this == Minecraft.getMinecraft().thePlayer) {
            EventStrafe strafeEvent = new EventStrafe(strafe, forward, friction);
            EventBus.getInstance().register(strafeEvent);
    ...
    {
    	//System.out.println(source.getEntity().getName());
