...
public class NetworkManager extends SimpleChannelInboundHandler<Packet>
{
	...
	protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet p_channelRead0_2_) {
        ...
        EventPacket eventPacket = new EventPacket(p_channelRead0_2_, EventPacket.Type.RECEIVE);

        EventBus.getInstance().register(eventPacket);

        if (eventPacket.isCancelled()){
            return;
        }

		EventPacketReceive packetRecieve = new EventPacketReceive(p_channelRead0_2_);
		EventBus.getInstance().register(packetRecieve);
    ...
    {
        EventPacket eventPacket = new EventPacket(packetIn, EventPacket.Type.SEND);
        EventBus.getInstance().register(eventPacket);
