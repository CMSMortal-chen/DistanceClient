...
import my.distance.module.modules.world.NoCommand;
...
public class EntityPlayerSP extends AbstractClientPlayer
{
     ...
     */
    public double getJumpHeight(double v1) {
        return isPotionActive(Potion.jump)?v1 + 0.1D * (double)(getActivePotionEffect(Potion.jump).getAmplifier() + 1):v1;
    }
    public double getBaseSpeed(double v1, double v3) {
        if(isPotionActive(Potion.moveSpeed)) {
            int a1 = getActivePotionEffect(Potion.moveSpeed).getAmplifier() + 1 - (isPotionActive(Potion.moveSlowdown)?getActivePotionEffect(Potion.moveSlowdown).getAmplifier() + 1:0);
    ...
    {
        EventMotion eventMotion = new EventMotion(EventMotion.Type.PRE);
        EventBus.getInstance().register(eventMotion);
        ...
        EventMotionUpdate event = new EventMotionUpdate(e,this.mc.thePlayer.posX, this.getEntityBoundingBox().minY, this.mc.thePlayer.posZ, this.mc.thePlayer.rotationYaw, this.mc.thePlayer.rotationPitch, this.mc.thePlayer.onGround, EventMotionUpdate.Type.PRE);
        ...
        this.lastReportedYaw = RotationUtils.serverRotation.getYaw();
        this.lastReportedPitch = RotationUtils.serverRotation.getPitch();
        if (RotationUtils.targetRotation != null) {
            this.rotationYaw = RotationUtils.targetRotation.getYaw();
            this.rotationPitch = RotationUtils.targetRotation.getPitch();
            Client.RenderRotate(rotationYaw, rotationPitch);
        } else if (oldPitch != e.getPitch() || oldYaw != e.getYaw()) {
            Client.RenderRotate(rotationYaw, rotationPitch);
        }else {
            Client.Pitch = 0;
        }
        ...
        event.setType(EventMotionUpdate.Type.POST);
        EventBus.getInstance().register(event);
        EventMotion eventMotionPost = new EventMotion(EventMotion.Type.POST);
        EventBus.getInstance().register(eventMotionPost);
        EventBus.getInstance().register(post);
    }
    ...
    public void sendChatMessage(String message) {
        if (message.startsWith(".") && !ModuleManager.getModuleByClass(NoCommand.class).isEnabled()) {
    ...
    protected boolean pushOutOfBlocks(double x, double y, double z)
    {
        ...
        EventBus.getInstance().register(epb);
        if (this.noClip)
        {
            return false;
        }
    ...
    public boolean isEntityInsideOpaqueBlock() {
        EventRenderBlock blockRenderEvent = new EventRenderBlock();
        EventBus.getInstance().register(blockRenderEvent);
    ...
    public void onLivingUpdate()
    {
        ...
        if (getHeldItem() != null && (this.isUsingItem() || (getHeldItem().getItem() instanceof ItemSword && ((KillAura)ModuleManager.getModuleByClass(KillAura.class)).getBlockingStatus() && !this.isRiding()))) {
            EventNoSlow eventSlowDown = new EventNoSlow(0.2F, 0.2F);
            EventBus.getInstance().register(eventSlowDown);
            this.movementInput.moveStrafe *= eventSlowDown.getMoveStrafe();
            this.movementInput.moveForward *= eventSlowDown.getMoveForward();
