...
public class FriendManager implements Manager {
	...
	public void init() {
		...
		Client.instance.getCommandManager().add(new FriendManager$1(this, "friend", new String[] { "f", "fren", "fr" },
				"add/del/list name alias", "管理客户端好友"));
	}
	...
	public String execute(String[] args) {
		...
		if (args.length >= 3) {
			if (args[0].equalsIgnoreCase("add")) {
				...
				if (FriendManager.access$0().size() > 0) {
					...
					for (var4 = FriendManager.access$0().values().iterator(); var4.hasNext(); ++var5) {
						fr = (String) var4.next();
						Helper.sendMessage(String.format("%s. %s", new Object[] {var5, fr }));
					}
				} else {
			...
			if (args[0].equalsIgnoreCase("add")) {
				...
				Helper.sendMessage(String.format("%s has been added", new Object[] { args[1], args[1] }));
				FileManager.save("Friends.txt", friends, true);
			} else if (args[0].equalsIgnoreCase("del")) {
				FriendManager.access$0().remove(args[1]);
				Helper.sendMessage(
