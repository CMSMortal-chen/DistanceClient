...
import my.distance.module.modules.render.Wings;
...
public class RenderWings extends  ModelBase {
   ...
   public RenderWings() {
      this.location = new ResourceLocation("Distance/ICON/wings.png");
   ...
   public void renderWings( float partialTicks) {
      ...
      double rotate = this.interpolate(per?mc.thePlayer.rotationYaw: ((ModuleManager.getModuleByClass(KillAura.class).isEnabled() && KillAura.currentTarget != null)? Client.Yaw: mc.thePlayer.prevRenderYawOffset), per?mc.thePlayer.rotationYaw:((ModuleManager.getModuleByClass(KillAura.class).isEnabled() && KillAura.currentTarget != null ||Client.instance.getModuleManager().getModuleByClass(Scaffold.class).isEnabled())?Client.Yaw: mc.thePlayer.renderYawOffset), partialTicks);
      GL11.glPushMatrix();
