...
public final class LiquidRender {
    ...
    public static void drawEntityBox(final Entity entity, final Color color, final boolean outline) {
        ...
        glColor(new Color(color.getRed(), color.getGreen(), color.getBlue(), outline ? 26 : color.getAlpha()));
        drawFilledBox(axisAlignedBB);
        GlStateManager.resetColor();
        glDepthMask(true);
        resetCaps();
    ...
    public static void renderNameTag(final String string, final double x, final double y, final double z) {
        ...
        final int width = FontLoaders.GoogleSans35.getStringWidth(string) / 2;

        Gui.drawRect(-width - 1, -1, width + 1, 17.5, Integer.MIN_VALUE);
        FontLoaders.GoogleSans35.drawStringWithShadow(string, -width, 1.5F, Color.WHITE.getRGB());
    ...
    public static void resetCaps() {
        glCapMap.forEach(LiquidRender::setGlState);
    }
