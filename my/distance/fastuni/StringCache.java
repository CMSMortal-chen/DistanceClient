...
public class StringCache
{
    ...
    {
        /* Change the font in the glyph cache and clear the string cache so all strings have to be re-layed out and re-rendered */
        glyphCache.setDefaultFont(font, fontSize, true);
    ...
    private int applyColorCode(int colorCode, int color, boolean shadowFlag)
    {
        ...
        if (colorCode != -1)
        {
            ...
            //color = RenderUtil.reAlpha(color,0.5f);
        }

        Tessellator.getInstance().getWorldRenderer().color(color >> 16 & 0xff, color >> 8 & 0xff, color & 0xff, shadowFlag ? 80:color >> 24 & 0xff);
        return color;
    }
